...
public class DetailController {
    ...
    private String detail(@RequestParam("id") Long id, @AuthenticationPrincipal CustomUserDetail userDetail, Model model) {
        ...
        // Tạo productInventoryMap để lưu số lượng tồn kho
        Map<Long, Integer> productInventoryMap = new HashMap<>();

        // Lấy số lượng tồn kho cho sản phẩm chính
        Inventory productInventory = inventoryRepository.findByProduct(product)
                .orElse(Inventory.builder().quantity(0).build());
        productInventoryMap.put(product.getId(), productInventory.getQuantity());

        // Lấy số lượng tồn kho cho featuredProducts
        for (Product featuredProduct : featuredProducts) {
            Inventory inventory = inventoryRepository.findByProduct(featuredProduct)
                    .orElse(Inventory.builder().quantity(0).build());
            productInventoryMap.put(featuredProduct.getId(), inventory.getQuantity());
        }

        // Lấy số lượng tồn kho cho relatedProducts
        for (Product relatedProduct : relatedProducts) {
            Inventory inventory = inventoryRepository.findByProduct(relatedProduct)
                    .orElse(Inventory.builder().quantity(0).build());
            productInventoryMap.put(relatedProduct.getId(), inventory.getQuantity());
        }
